...
def costs(request):
    ...
    if request.method == 'GET':
        ...
        cost = Cost(created_by = request.user, related_itinerary = itinerary)
        in_data = json.loads(request.body)
        participants = in_data.get('participant')
        partlist = []
        if participants :
            for participant in participants:
                partlist.append(participant.get('id'))
...
@login_required
@transaction.atomic
def costs_user(request):
    return HttpResponse(json.dumps(request.user.as_dict()), content_type='application/json')